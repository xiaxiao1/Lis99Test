...
//  0:已确认， 1：已拒绝， 2：待确认
    ...
    public void setType ( int t, int cId, int tId, ApplyManager mMain )
    {
        type = t;
        clubId = cId;
        topicId = tId;
...
//            holder.tv_title = (TextView) view.findViewById(R.id.tv_title);
        ...
        final ApplyManagerModel.Info.Lists item = (ApplyManagerModel.Info.Lists) getItem(i);

        if (item == null) return view;

        setIv_pay_state(item.pay_type, item.pay_status, holder.iv_pay_state, holder.tv_pay_state);
...
//        holder.tv_title.setText("报名信息" + (i + 1));
        ...
        for ( int n = 0; n < item.apply_info.size(); n++ )
        {
            ...
            list.add("报名信息" + (n + 1));
            HashMap<String, String> map = item.apply_info.get(n);
        ...
        ApplyManagerItem2 adapter = new ApplyManagerItem2(mContext, list);
