...
public class ChapterFragment extends Fragment {
	...
	public static ChapterFragment getNewInstance(int startPos, ArrayList<ShaiTuBean> list) {
		Utils.logh(TAG, "getNewInstance startPos: " + startPos + " list(" + list.size() + ") ");
		ChapterFragment fragment = new ChapterFragment();
		Bundle b = new Bundle();
		b.putParcelableArrayList(BUNDLE_KEY_NAME_LIST, list);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mNameList = args.getParcelableArrayList(BUNDLE_KEY_NAME_LIST);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		gv.setAdapter(new ChapterGvAdapter(mNameList));
	...
	private class ChapterGvAdapter extends BaseAdapter {
		...
		public ChapterGvAdapter(ArrayList<ShaiTuBean> mNameList) {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if(null == convertView) {
				...
				holder.mIcon = (AsyncLoadImageView) convertView.findViewById(R.id.cpt_gv_item_img);
			...
			if(lll!=null && lll.get(position)!=null){
				LinearLayout.LayoutParams ll = (android.widget.LinearLayout.LayoutParams) holder.mIcon.getLayoutParams();
				ll.width = StringUtil.getXY(getActivity())[0] / 3 - 10;
				ll.height = ll.width;
				holder.mIcon.setLayoutParams(ll);
				holder.mIcon.setImage(lll.get(position).getImage_url(), null, null);
			}
			return convertView;
		}
	...
	public interface onChapterPageClickListener {
		public abstract void onChapterItemClick(int position, ShaiTuBean name);
